...
export default function OrderHistory() {
  ...
  const getStatusBadge = (status) => {
    switch (status?.toLowerCase()) {
      case 'delivered':
        return <span className="text-[10px] uppercase font-bold text-[#388E3C] bg-emerald-50 px-2 py-1 rounded">Delivered ✔</span>;
      case 'cancelled':
            ...
            >
              ...
              <div className="flex-1 flex gap-2 overflow-x-auto no-scrollbar items-center py-1">
                {ord.items?.map((item, idx) => {
                  ...
                  return (
                    <div key={idx} className="relative h-12 w-12 border p-0.5 rounded-sm bg-white shrink-0 shadow-3xs hover:border-slate-300 transition select-none">
                      <img 
